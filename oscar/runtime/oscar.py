...
from .drivers import EFF, ECH4, EN2O, LUC, HARV, SHIFT, EHFC, EPFC, EODS, ENOX, ECO, EVOC, ESO2, ENH3, EOC, EBC, RFcon, RFvolc, RFsolar
...
from .oscar_data import EBC, ECH4, ECO, EFF, EHFC, EN2O, ENH3, ENOX, EOC, EODS, EPFC, ESO2, EVOC, HARV, LUC, RFcon, RFsolar, RFvolc, SHIFT
from .oscar_param import AERh_0, AREA_0, AWET_0, CFROZ_0, CH4_0, EESC_x, Gamma_BC, Gamma_DUST, Gamma_NO3, Gamma_O3s, Gamma_O3t, Gamma_POA, Gamma_SALT, Gamma_SO4, Gamma_SOA, N2O_0, Phi_0, alpha_BB_BC, alpha_BB_CH4, alpha_BB_CO, \
    alpha_BB_CO2, \
    alpha_BB_N2O, alpha_BB_NH3, alpha_BB_NOX, alpha_BB_OC, alpha_BB_SO2, alpha_BB_VOC, alpha_Br, alpha_CH4, alpha_CO2, alpha_HFC, alpha_LCC, alpha_N2O, alpha_NO3, alpha_ODS, alpha_OHC, alpha_PFC, alpha_POM, alpha_SO4, alpha_dic, \
    alpha_gyp, \
    alpha_mld, beta_gyp, chi_O3s_EESC, chi_O3s_N2O, chi_O3t_CH4, chi_O3t_CO, chi_O3t_NOX, chi_O3t_VOC, csoil1_0, csoil2_0, cveg_0, dty, ewet_0, f_RF_CH4, f_RF_CO2, f_RF_H2Os, f_RF_N2O, f_RF_overlap, f_fracrel, f_hv, f_kOH, f_npp, f_pCO2, \
    f_pH, f_rho, f_v_PF, gamma_igniC, gamma_igniP, gamma_igniT, gamma_mld, gamma_pthaw, gamma_rhoPF1, gamma_rhoPF2, gamma_wetC, gamma_wetP, gamma_wetT, igni_0, ind_final, k_BC_adjust, k_met, k_pthaw, lambda_0, mld_0, mu_0, n_Br, n_Cl, \
    nb_biome, nb_obox, nb_regionI, nb_regionPF, npp_0, p_AGB, p_HWP0, p_HWP1, p_HWP1_bb, p_HWP2, p_HWP3, p_OHC, p_PF1, p_PF2, p_PF3, p_PF_CH4, p_PF_inst, p_atm_CO2, p_atm_O3t, p_atm_absorb, p_atm_alb, p_atm_cloud, p_atm_noCO2, \
    p_atm_scatter, p_atm_solar, p_atm_strat, p_circ, p_reg4, p_reg9, p_wet, pthaw_min, r_HWP1, r_HWP2, r_HWP3, radeff_BC, radeff_BCsnow, radeff_DUST, radeff_HFC, radeff_NO3, radeff_O3s, radeff_O3t, radeff_ODS, radeff_PFC, radeff_POA, \
    radeff_SALT, radeff_SO4, radeff_SOA, rho1_0, rho2_0, solub_BC, solub_DUST, solub_NO3, solub_POA, solub_SALT, solub_SO4, solub_SOA, tau_BCbb, tau_BCff, tau_BVOC, tau_CH4_OH, tau_CH4_hv, tau_CH4_ocean, tau_CH4_soil, tau_DMS, tau_DUST, \
    tau_HFC_OH, tau_HFC_hv, tau_HFC_othr, tau_N2O_hv, tau_NH3, tau_NOX, tau_ODS_OH, tau_ODS_hv, tau_ODS_othr, tau_OMbb, tau_OMff, tau_PF1, tau_PF2, tau_PF3, tau_PFC_OH, tau_PFC_hv, tau_PFC_othr, tau_SALT, tau_SO2, tau_VOC, tau_circ, \
    tau_gst, tau_gst0, tau_lag, tau_shift, theta_0, v_fg, w_reg_BC, w_reg_BCsnow, w_reg_CO, w_reg_NOX, w_reg_OC, w_reg_SO2, w_reg_VOC, w_reg_lst, w_reg_lstPF, w_reg_lyp, w_reg_sst, w_rhoPF, warmeff_BCsnow, warmeff_LCC, warmeff_volc
...
class Simulation:
    ...
    def __init__(self, p=p, fT=fT, force_CO2=False, force_GHG=False, force_halo=False, force_RF=False, force_RFs=False, force_clim=False, D_CO2_force=None, D_CH4_force=None,
                 D_N2O_force=None, D_HFC_force=None, D_PFC_force=None, D_ODS_force=None, RF_CO2_force=None, RF_CH4_force=None, RF_H2Os_force=None, RF_N2O_force=None, RF_halo_force=None, RF_O3t_force=None, RF_O3s_force=None,
                 RF_SO4_force=None, RF_POA_force=None, RF_BC_force=None, RF_NO3_force=None, RF_SOA_force=None, RF_DUST_force=None, RF_SALT_force=None, RF_cloud_force=None, RF_BCsnow_force=None, RF_LCC_force=None, D_gst_force=None,
                 D_sst_force=None, D_lst_force=None, D_lyp_force=None, RF_force=None):
        self.p = p
        self.dt = 1 / self.p
        self.fT = fT
        self.force_CO2 = force_CO2
        self.force_GHG = force_GHG
        self.force_halo = force_halo
        self.force_RF = force_RF
        self.force_RFs = force_RFs
        self.force_clim = force_clim
        ...
        # Forces
        self.D_CO2_force = self.D_CO2 * 0 if D_CO2_force is None else D_CO2_force
        self.D_CH4_force = self.D_CH4 * 0 if D_CH4_force is None else D_CH4_force
        self.D_N2O_force = self.D_N2O * 0 if D_N2O_force is None else D_N2O_force
        self.D_HFC_force = self.D_HFC * 0 if D_HFC_force is None else D_HFC_force
        self.D_PFC_force = self.D_PFC * 0 if D_PFC_force is None else D_PFC_force
        self.D_ODS_force = self.D_ODS * 0 if D_ODS_force is None else D_ODS_force

        mk_scalar_var = lambda value=0: np.array([value], dtype=dty)
        self.RF_force = mk_scalar_var() if RF_force is None else RF_force
        self.RF_CO2_force = self.D_CO2 * 0 if RF_CO2_force is None else RF_CO2_force
        self.RF_CH4_force = self.D_CH4 * 0 if RF_CH4_force is None else RF_CH4_force
        self.RF_H2Os_force = mk_scalar_var() if RF_H2Os_force is None else RF_H2Os_force
        self.RF_N2O_force = self.D_N2O * 0 if RF_N2O_force is None else RF_N2O_force
        self.RF_halo_force = mk_scalar_var() if RF_halo_force is None else RF_halo_force
        self.RF_O3t_force = mk_scalar_var() if RF_O3t_force is None else RF_O3t_force
        self.RF_O3s_force = self.D_O3s * 0 if RF_O3s_force is None else RF_O3s_force
        self.RF_SO4_force = mk_scalar_var() if RF_SO4_force is None else RF_SO4_force
        self.RF_POA_force = mk_scalar_var() if RF_POA_force is None else RF_POA_force
        self.RF_BC_force = mk_scalar_var() if RF_BC_force is None else RF_BC_force
        self.RF_NO3_force = mk_scalar_var() if RF_NO3_force is None else RF_NO3_force
        self.RF_SOA_force = mk_scalar_var() if RF_SOA_force is None else RF_SOA_force
        self.RF_DUST_force = mk_scalar_var() if RF_DUST_force is None else RF_DUST_force
        self.RF_SALT_force = mk_scalar_var() if RF_SALT_force is None else RF_SALT_force
        self.RF_cloud_force = mk_scalar_var() if RF_cloud_force is None else RF_cloud_force
        self.RF_BCsnow_force = mk_scalar_var() if RF_BCsnow_force is None else RF_BCsnow_force
        self.RF_LCC_force = mk_scalar_var() if RF_LCC_force is None else RF_LCC_force

        self.D_gst_force = self.D_gst if D_gst_force is None else D_gst_force
        self.D_sst_force = self.D_sst if D_sst_force is None else D_sst_force
        self.D_lst_force = self.D_lst if D_lst_force is None else D_lst_force
        self.D_lyp_force = self.D_lyp if D_lyp_force is None else D_lyp_force

        self.tracking = []
        self.values_timeseries = []

    def track(self, variables):
        self.tracking[:] = list(set(variables))
    ...
    def output(self, vars):
        output = []
        for var in vars:
            output.append(getattr(self, var + '_t'))  # return output

    def step(self, t):
        for _ in range(p):
            self.small_step(t)

        timeseries = [getattr(self, name + '_t') for name in self.tracking]
        values = [getattr(self, name) for name in self.tracking]
        for arr, run in zip(timeseries, values):
            arr[t] += self.dt * run

    def small_step(self, t):
        self.step_ocean(t)
        self.step_land(t)
    ...
    def step_atmosphere(self, t):
        ...
        # FORCE
        if self.force_CO2:
            self.D_CO2 = self.D_CO2_force[t]

        if self.force_GHG:
            self.D_CO2 = self.D_CO2_force[t]
            self.D_CH4 = self.D_CH4_force[t]
            self.D_N2O = self.D_N2O_force[t]

        if self.force_halo:
            self.D_HFC[:] = self.D_HFC_force[t]
            self.D_PFC[:] = self.D_PFC_force[t]
            self.D_ODS[:] = self.D_ODS_force[t]

    def step_climate(self, t):
        dt = self.dt

        # Fluxes, per component
        ...
        # FORCE
        if self.force_RFs:
            self.RF_CO2 = self.RF_CO2_force[t]
            self.RF_CH4 = self.RF_CH4_force[t]
            self.RF_H2Os = self.RF_H2Os_force[t]
            self.RF_N2O = self.RF_N2O_force[t]
            self.RF_halo = self.RF_halo_force[t]
            self.RF_O3t = self.RF_O3t_force[t]
            self.RF_O3s = self.RF_O3s_force[t]
            self.RF_SO4 = self.RF_SO4_force[t]
            self.RF_POA = self.RF_POA_force[t]
            self.RF_BC = self.RF_BC_force[t]
            self.RF_NO3 = self.RF_NO3_force[t]
            self.RF_SOA = self.RF_SOA_force[t]
            self.RF_DUST = self.RF_DUST_force[t]
            self.RF_SALT = self.RF_SALT_force[t]
            self.RF_cloud = self.RF_cloud_force[t]
            self.RF_BCsnow = self.RF_BCsnow_force[t]
            self.RF_LCC = self.RF_LCC_force[t]

        # totals
        RF = self.RF_CO2 + self.RF_CH4 + self.RF_H2Os + self.RF_N2O + self.RF_halo + self.RF_O3t + self.RF_O3s + self.RF_SO4 + self.RF_POA + self.RF_BC + self.RF_NO3 + self.RF_SOA + self.RF_DUST + self.RF_SALT + self.RF_cloud + self.RF_BCsnow + self.RF_LCC + RFcon[t] + RFvolc[t] + RFsolar[t]
        ...
        # FORCE
        if self.force_RF:
            self.RF_warm = self.RF_force[t] * (self.RF_warm / RF)
            self.RF_atm = self.RF_force[t] * (self.RF_atm / RF)
            self.RF = self.RF_force[t]
        ...
        # FORCE
        if self.force_clim:
            self.D_gst = self.D_gst_force[t]
            self.D_sst = self.D_sst_force[t]
            self.D_lst = self.D_lst_force[t]
            self.D_lyp = self.D_lyp_force[t]

    def plot_variables(self, plot):
        plot_lst = []
        if plot == "all" or plot == "CO2" or "CO2" in plot:
            plot_lst += ["D_CO2", "OSNK", "LSNK", "ELUC", "D_AREA", "D_npp", "D_efire", "D_fmort", "D_rh1", "D_fmet", "D_rh2", "D_FIN", "D_FOUT", "D_FCIRC", "EFIRE_luc", "FMORT_luc", "RH1_luc", "FMET_luc", "RH2_luc",
                         "EHWP1_luc", "EHWP2_luc", "EHWP3_luc"]
        if plot == "all" or plot == "CH4" or "CH4" in plot:
            plot_lst += ["D_CH4", "D_OHSNK_CH4", "D_HVSNK_CH4", "D_XSNK_CH4", "D_EWET", "D_EBB_CH4"]
        if plot == "all" or plot == "N2O" or "N2O" in plot:
            plot_lst += ["D_N2O", "D_HVSNK_N2O", "D_EBB_N2O"]
        if plot == "all" or plot == "O3" or "O3" in plot:
            plot_lst += ["D_O3t", "D_O3s", "D_EESC", "D_N2O_lag", "D_gst"]
        if plot == "all" or plot == "AER" or "AER" in plot:
            plot_lst += ["D_SO4", "D_POA", "D_BC", "D_NO3", "D_SOA", "D_AERh", "RF_SO4", "RF_POA", "RF_BC", "RF_NO3", "RF_SOA", "RF_cloud", ]
        if plot == "all" or plot == "clim" or "clim" in plot:
            plot_lst += ["RF", "D_gst", "D_gyp", "RF_CO2", "RF_CH4", "RF_H2Os", "RF_N2O", "RF_halo", "RF_O3t", "RF_O3s", "RF_SO4", "RF_POA", "RF_BC", "RF_NO3", "RF_SOA", "RF_cloud", "RF_BCsnow", "RF_LCC"]
        return plot_lst

    def plot(self, plot):
        from ..plot import plot_AER, plot_CH4, plot_clim, plot_CO2, plot_N2O, plot_O3

        plot_lst = self.plot_variables(plot)
        print(plot_lst)
        if "D_CO2" in plot_lst:
...
def _OSCAR_lite(var_output=["D_CO2", "D_CH4", "D_N2O", "RF_halo", "D_O3t", "D_O3s", "D_SO4", "D_POA", "D_BC", "D_NO3", "D_SOA", "D_AERh", "RF", "D_gst"], plot="all"):
    sim = Simulation()
...
# self.step_land_use
# self.step_ocean
# self.step_permafrost
# self.tracking
# self.values_timeseries

from .old_run import OSCAR_lite