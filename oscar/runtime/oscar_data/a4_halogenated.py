...
from oscar.data import load_data_and_header, load_data
...
for VAR in PFC:
    ...
    TMP = load_data(path)
    for i in range(114 + 1):
        EPFCedgar[270: ind_edgar + 1, regionJ_index[i], 0, kindGHG_index["PFC"], regionI_index[i], PFC.index(VAR)] += \
...
TMP, lgd = load_data_and_header(path)
for x in range(len(lgd)):
    EODScmip5[306: ind_cdiac + 1, 0, 0, kindGHG_index["ODS"], 0, ODS.index(lgd[x])] = \
        TMP[6: min(ind_final, ind_cdiac) - 300 + 1, x]
...
if data_Ehalo == "EDGAR":
    ...
    for t in range(ind_edgar + 1, ind_cdiac + 1):
        ...
        EHFCpast[t, ...] *= np.sum(EHFCpast[t - 1, ...]) / np.sum(EHFCpast[t, ...]) * np.sum(EHFCeft[t, ...]) / np.sum(
            EHFCeft[t - 1, ...])
    EHFCpast[np.isnan(EHFCpast) | np.isinf(EHFCpast)] = 0
...
if data_Ehalo == "EDGAR":
    ...
    for t in range(ind_edgar + 1, ind_cdiac + 1):
        ...
        EPFCpast[t, ...] *= np.sum(EPFCpast[t - 1, ...]) / np.sum(EPFCpast[t, ...]) * np.sum(EPFCeft[t, ...]) / np.sum(
            EPFCeft[t - 1, ...])
    EPFCpast[np.isnan(EPFCpast) | np.isinf(EPFCpast)] = 0
    # quadratic extrapolation before 1970
    # starting year of emission based on [Meinshausen et al., 2011]
        ...
        # follow trends of projection
            ...
            for t in range(ind_cdiac + 1, ind_final + 1):
                ...
                if def_regI and def_regJ:
                    ...
                    arr[t, :, ..., 0] = np.sum(arr[t - 1, :, ..., :], 0) * np.sum(arrproj[t, :, ..., :], -1) / np.sum(
                        arrproj[t - 1, :, ..., :], -1)
                    arr[np.isnan(arr) | np.isinf(arr)] = 0
                    arr[t, ...] *= np.sum(arr[t - 1, ...]) / np.sum(arr[t, ...]) * np.sum(arrproj[t, ...]) / np.sum(
                        arrproj[t - 1, ...])
