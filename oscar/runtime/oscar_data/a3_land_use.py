...
if data_LULCC[:3] == "LUH":
    for b in range(len(bio)):
        path = f"data/LandUse_{data_LULCC}/#DATA.LandUse_{data_LULCC}_{mod_LSNKcover}.1501-2015_114reg1.HARV_" \
               f"{bio[b]}.csv"
...
if data_LULCC[:3] == "LUH":
    for b1 in range(len(bio)):
        for b2 in range(b1, len(bio)):
            path = f"data/LandUse_{data_LULCC}/#DATA.LandUse_{data_LULCC}_{mod_LSNKcover}.1501-2015_114reg1.SHIFT_" \
                   f"{bio[b1]}2{bio[b2]}.csv"
            if os.path.isfile(path):
                TMP = load_data(path)
                for i in range(1, 114 + 1):
...
if (scen_LULCC[:3] == "RCP") & (ind_final > ind_cdiac):
    for b1 in range(len(bio)):
        for b2 in range(len(bio)):
            path = f"data/LandUse_RCP/#DATA.LandUse_RCP_{mod_LSNKcover}.2006-2100_114reg1.rcp{scen_LULCC[3]}" \
                   f"{scen_LULCC[5]}_LUC_{bio[b1]}2{bio[b2]}.csv"
...
if (scen_LULCC[:3] == "RCP") & (ind_final > ind_cdiac):
    for b in range(len(bio)):
        path = f"data/LandUse_RCP/#DATA.LandUse_RCP_{mod_LSNKcover}.2006-2100_114reg1.rcp{scen_LULCC[3]}" \
               f"{scen_LULCC[5]}_HARV_{bio[b]}.csv"
...
if (scen_LULCC[:3] == "RCP") & (ind_final > ind_cdiac):
    for b1 in range(len(bio)):
        for b2 in range(b1, len(bio)):
            path = f"data/LandUse_RCP/#DATA.LandUse_RCP_{mod_LSNKcover}.2006-2100_114reg1.rcp{scen_LULCC[3]}" \
                   f"{scen_LULCC[5]}_SHIFT_{bio[b1]}2{bio[b2]}.csv"
