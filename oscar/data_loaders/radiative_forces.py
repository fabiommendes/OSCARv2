...
def _load_rf(which):
    data = np.zeros([conf.ind_final + 1, nb_regionJ, nb_sector, nb_kind], dtype=conf.dty)
    if conf.data_RFant != "IPCC-AR5":
        return data
    ...
    # Past estimates
    i = lgd.index(which)
    data[51: min(conf.ind_final, ind_cdiac) + 1, 0, 0, _kind_idx[which]] += aux[1: 1 + min(conf.ind_final, ind_cdiac) - 51 + 1, i]

    # Arbitrary extension
    if (conf.scen_RFant == "cst") & (conf.ind_final > ind_cdiac):
