...
RF_rcp = np.zeros([800 + 1, 6], dtype=conf.dty)
RF_rcp[:300] = np.nan
n = -1
...
def f_RF_CO2(D_CO2):
    ...
    return np.array(RF, dtype=conf.dty)
...
radeff_HFC = 1e-3 * np.array([0.18, 0.11, 0.23, 0.16, 0.16, 0.10, 0.26, 0.24, 0.24, 0.22, 0.42], dtype=conf.dty)
radeff_PFC = 1e-3 * np.array([0.57, 0.20, 0.09, 0.25, 0.28, 0.32, 0.36, 0.41, 0.44, 0.50], dtype=conf.dty)
radeff_ODS = 1e-3 * np.array([0.26, 0.32, 0.30, 0.31, 0.20, 0.17, 0.07, 0.21, 0.16, 0.19, 0.29, 0.27, 0.30, 0.31, 0.004, 0.01], dtype=conf.dty)
...
radeff_O3t = radeff_O3t_map[conf.mod_O3Tradeff]
...
radeff_O3s = radeff_O3s_map[conf.mod_O3Sradeff]
...
radeff_SO4 = radeff_SO4_map[conf.mod_SO4radeff]
...
radeff_POA = radeff_POA_map[conf.mod_POAradeff]
...
radeff_BC = radeff_BC_map[conf.mod_BCradeff]
...
radeff_NO3 = radeff_NO3_map[conf.mod_NO3radeff]
...
radeff_SOA = radeff_SOA_map[conf.mod_SOAradeff]
...
k_BC_adjust = k_BC_adjust_map[conf.mod_BCadjust]

# solubility of aerosols for the aerosol-cloud interaction
# from [Hansen et al., 2005]
if conf.mod_CLOUDsolub == "Hansen2005":
    solub_SO4 = 1.0
    solub_POA = 0.8
...
elif conf.mod_CLOUDsolub == "Lamarque2011":
    solub_SO4 = 1.0
    solub_POA = 0.86
    solub_BC = 0.80
    solub_NO3 = 1.0
...
RF_ref1 = RF_ref1_map[conf.mod_CLOUDerf]

# soluble aerosol load for the aerosol-cloud interaction
# load pre-processed ACCMIP config.data_loaders
path = f"data/AeroCloud_ACCMIP/#DATA.AeroCloud_{conf.mod_CLOUDerf}.(2yr)_(7aer).LOAD.csv"
TMP, lgd = load_data_and_header(path)
del lgd[0]
AER_ref0 = 0
AER_ref1 = 0
...
AERh_0 = AERh_0_map[conf.mod_CLOUDpreind]
...
p_reg9 = np.zeros([nb_regionI, 9 + 1], dtype=conf.dty)
...
if conf.mod_ALBBCreg == "Reddy2007":
    w_reg_BCsnow = np.array([1, 1 / 6.0, 11 / 11.0, 1 / 10.0, 63 / 12.0, 2 / 3.0, 2 / 13.0, 17 / 43.0, 1 / 1.0, 2 / 1.0], dtype=conf.dty)
...
radeff_BCsnow = radeff_BCsnow_map[conf.mod_ALBBCrf]
...
warmeff_BCsnow = warmeff_BCsnow_map[conf.mod_ALBBCwarm]
...
path = f"data/Albedo_{conf.mod_ALBLCalb}/#DATA.Albedo_{conf.mod_ALBLCalb}_{conf.mod_ALBLCflux}_{conf.mod_ALBLCcover}.114reg1_7bio.alb.csv"
path2 = f"data/Albedo_{conf.mod_ALBLCalb}/#DATA.Albedo_{conf.mod_ALBLCalb}_{conf.mod_ALBLCflux}_{conf.mod_ALBLCcover}.114reg1_7bio.RSDS.csv"
TMP = load_data(path)
...
path = f"data/RadFlux_{conf.mod_ALBLCflux}/#DATA.RadFlux_{conf.mod_ALBLCflux}.114reg1.rsds.csv"
path2 = f"data/RadFlux_{conf.mod_ALBLCflux}/#DATA.RadFlux_{conf.mod_ALBLCflux}.114reg1.AREA.csv"
TMP = load_data(path)
TMP2 = load_data(path2)
...
warmeff_LCC = warmeff_LCC_map[conf.mod_ALBLCwarm]