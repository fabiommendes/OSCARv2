...
from .radiative_forces import f_RF_CO2
...
tau_gst = tau_Tfast * tau_Tslow / (tau_Tfast * p_Tslow + tau_Tslow * p_Tfast)
tau_gst0 = tau_Tfast * p_Tfast + tau_Tslow * p_Tslow - tau_gst
theta_0 = tau_gst0 / (tau_Tfast * p_Tslow + tau_Tslow * p_Tfast)
...
alpha_gyp, beta_gyp = fmin(err, [10.0, -50.0], disp=False)
alpha_gyp = np.abs(alpha_gyp)
beta_gyp = -np.abs(beta_gyp) / f_RF_CO2(3 * CO2_0)
...
if config.mod_PRECradfact == "Andrews2010":
    ...
    p_atm_noCO2 = 0.5
    p_atm_O3t = -0.3
    p_atm_strat = 0.0  # assumed
    p_atm_scatter = 0.0
    p_atm_absorb = 2.5
    p_atm_cloud = 0.0  # assumed
    p_atm_alb = 0.0
    p_atm_solar = 0.2
...
# assumptions following [Allan et al., 2014] and nil if not given
    ...
    p_atm_strat = 0.0  # assumed
    p_atm_scatter = 0.6 / -1.4
    p_atm_absorb = 3.1 / 0.5
    p_atm_cloud = 0.0  # assumed
    p_atm_alb = 0.0  # not given
    p_atm_solar = 0.4 / 3.5
...
w_reg_sst = fmin(err, [1], disp=False)[0]
...
if config.mod_PRECpattern == "4xCO2":
    ...
    gyp_ctrlPR, pr_ctrlPR, AREA_ctrlPR = _load_mod_prec("ctrl")
    gyp_quadPR, pr_quadPR, AREA_quadPR = _load_mod_prec("quad")
