...
from ..params import alpha_CO2, alpha_CH4, alpha_N2O, alpha_HFC, alpha_PFC, alpha_ODS, tau_lag
...
class AtmosphereSimulatorMixin:
    ...
    D_CO2 = scalar_var()
    D_CH4 = scalar_var()
    D_CH4_lag = scalar_var()
    D_N2O = scalar_var()
    D_N2O_lag = scalar_var()
    D_HFC = linear_var(nb_HFC)
    D_HFC_lag = linear_var(nb_HFC)
    D_PFC = linear_var(nb_PFC)
    D_PFC_lag = linear_var(nb_PFC)
    D_ODS = linear_var(nb_ODS)
    D_ODS_lag = linear_var(nb_ODS)

    D_CO2_t = scalar_timeseries()
    D_CH4_t = scalar_timeseries()
    D_CH4_lag_t = scalar_timeseries()
    D_N2O_t = scalar_timeseries()
    D_N2O_lag_t = scalar_timeseries()
